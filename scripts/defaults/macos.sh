...
LOCAL_HOST_NAME="$(printf '%s' "$COMPUTER_NAME" | tr -cd '[:alnum:]-' | cut -c 1-63)"
...
readonly DOTFILES_ROOT="${BASE_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)}"
...
write_default_array() {
  ...
  current=$(defaults read "$domain" "$key" 2>/dev/null | tr -d '[:space:];"(),' || true)
  desired=$(printf '%s' "$*" | tr -d '[:space:];"(),')
...
set_computer_name() {
  if [[ -z "$COMPUTER_NAME" ]]; then
    log_info "Computer name not provided; leaving host names unchanged"
    return 0
  fi

  log_info "Configuring computer name to '$COMPUTER_NAME' and local host name to '$LOCAL_HOST_NAME'"
  run_or_warn sudo scutil --set ComputerName "$COMPUTER_NAME"
  run_or_warn sudo scutil --set HostName "$COMPUTER_NAME"
  run_or_warn sudo scutil --set LocalHostName "$LOCAL_HOST_NAME"
